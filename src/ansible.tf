resource "local_file" "inventory" {
  filename = "for.ini"
  content  = templatefile("ansible-inventory.tpl", { webservers = yandex_compute_instance.web, databases = yandex_compute_instance.db, storage = yandex_compute_instance.storage.* })
}
...
resource "null_resource" "web_hosts_provision" {
  ...
  depends_on = [yandex_compute_instance.db, yandex_compute_instance.storage]
